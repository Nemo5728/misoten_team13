...
public class loginArea : MonoBehaviour
{
    ...
    private TSVector targetPos;//プレイヤーの位置を入れる
    ...
    void Start()
    {
        ...
        //ターゲット生成
        targetPos =  new TSVector(0.0f, 0.0f, 0.0f);//位置初期化
    ...
    void Update()
    {
        ...
        {
            //当り判定：床の上に乗っているなら
            if (posAreaTex.x + AreaSize > targetPos.x &&
            posAreaTex.x - AreaSize < targetPos.x &&
            posAreaTex.z + AreaSize > targetPos.z &&
            posAreaTex.z - AreaSize < targetPos.z)
            {
                bPlayerArea = true;
            }
