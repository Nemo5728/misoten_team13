...
_ClockCount("_ClockCount", Range(0,60) ) = 0
_ClockLight("_ClockLight", Range(0,1) ) = 0
...
float4 Ceil0=ceil(_ClockCount.xxxx);
float4 Add10=Ceil0 + float4( -1,-1,-1,-1 );
float4 UV_Pan1=float4((IN.uv_BaseTex.xyxy).x,(IN.uv_BaseTex.xyxy).y + Add10.y,(IN.uv_BaseTex.xyxy).z,(IN.uv_BaseTex.xyxy).w);
...
float4 Multiply6=Add13 * _ClockLight.xxxx;
float4 Add11=Add12 + Multiply6;
float4 Sampled2D1=tex2D(_noiseMask,IN.uv_noiseMask.xy);
float4 Add6=Add11 + Sampled2D1;
float4 Multiply0=_loadUVpan.xxxx * float4( -65,-65,-65,-65 );
float4 Add0=Multiply0 + float4( 61,61,61,61 );
